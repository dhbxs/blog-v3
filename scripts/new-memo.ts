...
const dir = join('content', 'memos', now.year.toString())
...
const permalink = usePermalink
	? `/memos/${randomBytes(4).toString('hex').slice(1)}`
	: undefined
...
do {
	...
	title = normalize(await text({
		message: '请输入说说标题-页面不显示标题，仅供后续整理使用',
